...
- desk_satellite.ino (containing setup() and loop(), plus #includes from other files)
...
- Sign up for a personal GitHub account at https://github.com/signup using any email address you can access (your school email, QQ mail, Gmail — anything works).
- Enable two-factor authentication (2FA) on your account (Settings → Password and authentication). GitHub requires this, and it protects your work.
- Check (Settings → Emails) that your email is verified.
...
- Decide on your team (1–2 people). Pick a team name, e.g. desk-satellite-team3.
- One member creates a new repository on GitHub:
...
VS Code “Git: Clone” command palette
...
GitHub green “Code” button with HTTPS URL
...
## Submission
(R1) Once you have set up your GitHub account and created/cloned your team repository, add your GitHub username(s), team name, and repository URL here in the README.md file.
# Task 3: Git Basics
...
VS Code Source Control panel — stage and commit

VS Code Source Control panel — sync and push
...
- GitHub Skills — Get started with Git and GitHub
- A 1-2 hour crash course from Class Central (freeCodeCamp)
- Atlassian Git Tutorial
- Connecting to GitHub with SSH
- Prefer video? Search Bilibili (recommended in China) or YouTube for “GitHub SSH setup tutorial” and pick a recent one in your preferred language.
## Submission
...
Example git commit history
(S1) Include a screenshot of your commit history, similar to the one above, after committing with your filled assignment header.
See the Appendix for information on including a screenshot in a markdown file in VS Code.

# Task 3B: ESP32-C3 SuperMini Plus Blink Test
This mini-lab goes after the VS Code / Git workflow section. Students will create a small Arduino sketch, keep it in the team repository, upload it to the ESP32-C3 board, and verify a basic GPIO output.
## Goal and Materials
Goal: use GPIO4 to turn an external LED on and off every 0.5 seconds.
Materials: ESP32-C3 SuperMini Plus, one LED, one 220 Ω or 330 Ω resistor, breadboard, jumper wires, and a USB-C data cable.
## Put the Code in VS Code / GitHub
Create a small folder for this test inside your team repository. You may upload from Arduino IDE, but the code file should still be saved and committed through VS Code / Git.
esp32c3_blink/
  external_led_gpio4/
    external_led_gpio4.ino
  README.md
## Arduino IDE Setup
Before uploading, check only these settings:
Board: ESP32C3 Dev Module.
Port: choose the ESP32 USB port, such as /dev/cu.usbmodem... on macOS or COMx on Windows. Do not choose Bluetooth or debug-console.
USB / upload: USB CDC On Boot: Enabled. Start with Upload Speed: 115200.
## External LED Wiring
Use GPIO4 for the first external LED test. The number in the code must match the physical GPIO pin used in the circuit.
GPIO4 -> 220 Ω / 330 Ω resistor -> LED long leg (+)
LED short leg (-) -> GND
Important: always use a resistor. ESP32-C3 GPIO is 3.3 V. Do not connect the LED directly to 5 V.
## Blink Code
Create a new Arduino sketch named external_led_gpio4.ino and paste this code:
...
## Upload Check
Click the Upload arrow. Do not click Debug or the bug icon. A successful upload normally shows:
Hash of data verified.
Hard resetting via RTS pin...
If the board does not start automatically, press RST once and check whether the LED blinks.
## Quick Troubleshooting
Stuck at Connecting... or No serial data received: hold BOOT, click Upload, and release BOOT when Writing starts. If needed, press RST while holding BOOT to force download mode.
esp32s3 / JTAG / gdb appears: the wrong board or wrong button was used. Select ESP32C3 Dev Module and click Upload, not Debug.
Cannot find the correct port: choose the usbmodem / COMx ESP32 port. Do not choose Bluetooth or debug-console.
LED does not blink: check LED polarity, resistor, GND, and whether LED_PIN matches the wired GPIO pin.
## Supplement: Built-in LED Test
To test the built-in LED instead, many ESP32-C3 SuperMini boards use GPIO8. The built-in LED is often active-low: LOW turns it on and HIGH turns it off.
#define LED_PIN 8   // built-in LED on many ESP32-C3 SuperMini boards
...
## Submission
If this mini-lab is graded, add these items to your team README.md and commit the sketch file to your repository:
(I2) Include a photo of your external LED circuit.
(S2) Include a screenshot of a successful Arduino IDE upload.
(C1) external_led_gpio4/external_led_gpio4.ino containing the blink code.
(T1) Demo the blinking external LED to a TA or instructor.
...
They can be overwhelming — where should you start to look for information? This How to Read a Datasheet PDF might help you. It breaks down various parts of a datasheet and highlights important aspects.

How to Read a Datasheet guide
Here’s another good guide: How to Read a Power MOSFET Datasheet.
...
For questions R2–R5, use this Electret Microphone datasheet (Same Sky Devices, manufacturer):
(R2) What is the operational temperature range that the manufacturer claims the product will remain functional?
(R3) What is the device frequency range?
(R4) What is the current consumption (and under what conditions)?
(R5) If you change the input voltage, will this current consumption remain the same? (Update: Since a “correct” answer may require some understanding of how Electret microphones function and is not directly explained in the datasheet, credit will be given as long as you explain your reasoning.)
(R6) We have a resistor RMCF0603FT2K20 and the RMCF/RMCP Series datasheet. What is the power rating for our resistor at 70˚C?
...
For questions R7–R8, use the ATmega328PB Datasheet (official Microchip product page: microchip.com/en-us/product/ATmega328PB). Refer to section 33, Electrical Characteristics.
(R7) What is the limiting DC current each I/O pin can source/sink? Given the MCU Vcc (high voltage level) is 5V, what is the smallest resistive load that can be connected to an I/O pin to exceed this rated limiting current when the I/O pin is driven high?
(R8) What is the leakage current for an I/O pin? What is a leakage current?
(R9) Given this Blue LED datasheet, what is the smallest value resistance that can be used for the current-limiting resistor if the driving voltage is 5V DC? Consider which forward-voltage value limit (min/max) defines your worst-case limited current and calculate the minimum resistance value that works for the worst case (and therefore other cases).
For R10 and R11, go to LCSC (lcsc.com) or Taobao and find parts that meet the following specifications. Provide the part number and/or a link to the product listing.
- We recommend filtering for in-stock parts that have an available datasheet.
(R10) Provide a through-hole 22 kΩ resistor with 5% tolerance or better, and a 0.5 W or higher power rating.
(R11) Provide a through-hole pushbutton with a current rating of at least 90 mA.
...
- All URLs must be clickable in the PDF format.
...
Name: Typora ID: cweijan.vscode-typora Publisher: Weijan Chen VS Marketplace Link: https://marketplace.visualstudio.com/items?itemName=cweijan.vscode-typora
...
MPE worked well on macOS and Windows, though you’ll need to have Chrome installed — or a generator like Prince.
Name: Markdown Preview Enhanced ID: shd101wyy.markdown-preview-enhanced Publisher: Yiyi Wang VS Marketplace Link: https://marketplace.visualstudio.com/items?itemName=shd101wyy.markdown-preview-enhanced
...
Name: vscode-pdf ID: tomoki1207.pdf Publisher: tomoki1207 VS Marketplace Link: https://marketplace.visualstudio.com/items?itemName=tomoki1207.pdf
## Submission flow
- Commit & push your final README.md (and any code/images) to your team GitHub repository.
- Export your README.md → PDF using one of the tools above.
- Email the PDF to the teaching team at teaching-team-email@example.com (the real address will be confirmed by your instructor) with the subject line:
...
## Submission
(R12) What are the six types of submission questions (R/I/S/C/V/T), and where does each one go?
(R13) What file format do you email to the teaching team, and how is it generated?
(R14) Where does your graded code live, and where does your graded write-up live?
# Submission Requirements
Your assignment is graded from two sources:
- The commits & README.md in your team GitHub repository, and
- The PDF you email to the teaching team.
...
- Save the image file (e.g., screenshot.png) in your repository folder (a media/ or images/ subfolder keeps things tidy).
- Reference it in Markdown using the image syntax:
...
- makeareadme.com — How to write a README
- Curated list of example READMEs (awesome-readme)
- Adobe free video-to-GIF converter
- VS Code is heavily recommended for developing code and handling Git commits.
